...
BEGIN
    ...
    PrescriptionDate DATETIME2(3) NOT NULL DEFAULT SYSUTCDATETIME(),
    Notes NVARCHAR(MAX),
    IsActive BIT NOT NULL DEFAULT 1,
    
    CONSTRAINT FK_Prescription_Elder 
        FOREIGN KEY (ElderID) REFERENCES dbo.elderTable(elderId),
        
    CONSTRAINT FK_Prescription_Caretaker 
        FOREIGN KEY (CareTakerID) REFERENCES dbo.caretakerTable(CareTakerID)
);
END
GO

/* 14. Add Prescription */
CREATE OR ALTER PROCEDURE sp_prescription_add
    @ElderID INT,
    @CaretakerID INT,
...
BEGIN
    SET NOCOUNT ON;
    DECLARE @PrescriptionID INT = 0;

    INSERT INTO Prescription (ElderID, CaretakerID, MedicineName, Dosage, Frequency, Notes, IsActive)
    VALUES (@ElderID, @CaretakerID, @MedicineName, @Dosage, @Frequency, @Notes, 1);

    SET @PrescriptionID = SCOPE_IDENTITY();
    SELECT 1 AS Status, @PrescriptionID AS PrescriptionID;
END
GO

/* 15. Get Prescriptions by Elder ID */
CREATE OR ALTER PROCEDURE sp_prescription_get_by_elder
    @ElderID INT
AS
BEGIN
    SET NOCOUNT ON;
    SELECT * FROM Prescription WHERE ElderID = @ElderID AND IsActive = 1;
END
GO

/* 16. Update Prescription */
CREATE OR ALTER PROCEDURE sp_prescription_update
    @PrescriptionID INT,
    @MedicineName NVARCHAR(255),
    @Dosage NVARCHAR(100),
    @Frequency NVARCHAR(100),
...
/* 17. Delete Prescription (Soft Delete) */
CREATE OR ALTER PROCEDURE sp_prescription_delete
    @PrescriptionID INT
AS
BEGIN
    SET NOCOUNT ON;
    UPDATE Prescription SET IsActive = 0 WHERE PrescriptionID = @PrescriptionID;
    SELECT 1 AS Status;
END
GO
